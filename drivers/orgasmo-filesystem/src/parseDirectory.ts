...
import * as pathRoRegExp from "path-to-regexp";
...
export default async function parseDirectory(pathToJsonDirectory) {
    ...
    const files = await glob(join(process.cwd(), pathToJsonDirectory, '/**/*.json'))
    ...
    for (const pagePath of files) {
        ...
        if (ids.has(pageConfig.id)) {
            console.error(`there is already a pageConfig with the id ${pageConfig.id}`)
            continue
        }
    ...
    sortedPaths.sort((a, b) => {
        const lastA = a.replace(/\/:/g, '/￾')  // unicode FFFE before last character
                       .replace(/\/:/g, '/￿'); // unicode FFFF last character
        const lastB = b.replace(/\/:/g, '/￾')  // unicode FFFE before last character
                       .replace(/\/:/g, '/￿'); // unicode FFFF last character
