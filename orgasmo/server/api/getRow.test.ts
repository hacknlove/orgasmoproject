...
describe("getRow", () => {
    ...
    it('calls driver.page.getPageFromId to get the row', async () => {
        ...
        expect(driver.page.getPageFromId).toHaveBeenCalledWith({ pageId: 'getRow-test' });
    ...
    it('chooses one if the row is an array', async () => {
        const driver = {
            ...
            'somePage.getRow': jest.fn(() => [{ type: 'test-1' }, { type: 'test-2' }]) 
        };
    ...
    it('serializes props.getMore if defined', async () => {
        const driver = {
            page: { getPageFromId: () => ({ getRow: 'somePage.getRow' }) },
            'somePage.getRow': jest.fn(() => ({ type: 'test-1', props: { getMore: { handler: 'test' } } }))
        };
