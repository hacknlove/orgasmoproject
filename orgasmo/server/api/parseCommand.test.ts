...
describe("parseCommand", () => {
  it("return an error if the signature is not valid and calls the driver", async () => {
    ...
    expect(events.emit).toHaveBeenCalledWith("badSigned", {
      req,
      command: {
  ...
  it("return an error if the signature is expired", async () => {
    ...
    expect(events.emit).toHaveBeenCalledWith("expiredSignature", {
      req,
      command: {
        expire: 0,
      },
  ...
  it("return an error if the user is not the same", async () => {
    ...
    expect(events.emit).toHaveBeenCalledWith("wrongUser", {
      req,
      command: {
        roles: "wrong user",
      },
